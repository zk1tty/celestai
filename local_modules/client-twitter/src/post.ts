...
Must be a singe sentence.

{{characterPostExamples}}`;

const MAX_TWEET_LENGTH = 280;

// Assuming elizaLogger.log supports styled output
const styledLog = (message: string) => {
    // Split the message by line breaks
    const lines = message.split('\n');

    // Apply styling to each line
    const styledLines = lines.map(line => {
        // Example: Add a prefix or style each line
        return `  ${line}`;
    });

    // Join the lines back together with line breaks
    const styledMessage = styledLines.join('\n');

    // Log the styled message
    console.log(styledMessage); // Example styling
};
