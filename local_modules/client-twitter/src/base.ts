...
export class ClientBase extends EventEmitter {
    ...
    async fetchHomeTimeline(count: number): Promise<Tweet[]> {
        ...
        homeTimeline?.tweets.forEach(elm => console.log("twitterUrl:", elm?.permanentUrl));
    ...
    // TODO: fetching latest mentioned tweets
    // fetch for interaction tweet candidates
    async fetchSearchTweets(
        ...
        try {
            console.log("query:", query, "maxTweets:", maxTweets, "cursor:", cursor);
            // Sometimes this fails because we are rate limited. in this case, we just need to return an empty array
            // if we dont get a response in 5 seconds, something is wrong
            const timeoutPromise = new Promise((resolve) =>
                setTimeout(() => resolve({ tweets: [] }), 20000)
            );
    ...
    private async populateTimeline() {
        elizaLogger.info("populating timeline...");
        ...
        if (cachedTimeline) {
            ...
            // TODO: wht is the existing memeories by roomIds
        ...
        // Cache: timeline, mentions, and query search with fetchSearchTweets
    ...
    async saveRequestMessage(message: Memory, state: State) {
        if (message.content.text) {
            ...
            if (
                ...
                elizaLogger.info("Message already saved", recentMessage[0].id);
            } else {
                await this.runtime.messageManager.createMemory({
                    ...message,
                    embedding: embeddingZeroVector,
